...
export function SceneTree({ biData, configEnv, viewer }: Props): JSX.Element {
  ...
  async function clickRow(e: MouseEvent | PointerEvent): Promise<void> {
    ...
    console.log(`${row?.suppliedId},${row?.name}`);
    ...
    <VertexSceneTree
      ...
      rowData={(row: Row) => {
        ...
        const data = {
          ...
          visible: row.visible,
          icon: svgPath(row.visible ? "shown" : "hidden"),
          style: "",
          value: "",
        };
...
function svgPath(icon: IconType): string {
  switch (icon) {
    case "hidden":
